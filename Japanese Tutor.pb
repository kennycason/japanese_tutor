...
srepeat = 0
  ...
  LanguageMode = 0 ; english / 1 japanese
  Gosub CreateToolbarEnglish
...
CreateToolbarJapanese:
   If CreateMenu(0, WindowID())
      MenuTitle("かな")
        OpenSubMenu("ひらがな")       ; begin sub-menu
          MenuItem( #HiraganaChart, "チャート")
          MenuItem( #HiraganaFlash, "フラシュカード")
          CloseSubMenu()
        OpenSubMenu("カタカナ")
          MenuItem(#KatakanaChart, "チャート") 
          MenuItem(#KatakanaFlash, "フラシュカード")
          CloseSubMenu()
      MenuTitle("漢字")  
          MenuItem(#KanjiChart, "チャート") 
          MenuItem(#KanjiFlash, "フラシュカード") 
      MenuTitle("言語")
        OpenSubMenu("グラマー")
          MenuItem(#Grammar,"動詞")
        CloseSubMenu()
        MenuItem(#TextConv, "かなコンヴォルション")
        MenuItem(#Dictionary, "辞書")
      MenuTitle("セテイング")
        OpenSubMenu("音")
          MenuItem(#OnOff, "あります/ありません")
          MenuItem(#Repeat, "リピート")
        CloseSubMenu()
        OpenSubMenu("言語/Language")
          MenuItem(#JapaneseMode, "日本語/Japanese")
          MenuItem(#EnglishMode,"ロマジ/English")
        CloseSubMenu()
      MenuTitle("ヘルプ")
        MenuItem(#Help, "ヘルプ")
        MenuItem(#Info, "インフォ")
...
CreateToolbarEnglish:
   If CreateMenu(0, WindowID())
      ...
      MenuTitle("Settings")
        OpenSubMenu("Sound")
          MenuItem(#OnOff, "Sound On/Off")
          MenuItem(#Repeat, "Repeat")
        CloseSubMenu()
        OpenSubMenu("Language")
          MenuItem(#JapaneseMode, "Japanese")
          MenuItem(#EnglishMode,"English")
        CloseSubMenu()
      MenuTitle("Help")
        MenuItem(#Help, "Help")
        MenuItem(#Info, "Info")
...
HiraganaChart:
    ...
    If CreateGadgetList(WindowID())
      ...
      Else 
        t$ = "ひらがなチャート"
      EndIf
      TextGadget(#GADGET_Text,WindowWidth/2-70, top-20,100,15,t$)
      k = 1
...
HiraganaFlash:
    ...
    If CreateGadgetList(WindowID())
      ...
      Else 
        t$ = "ひらがなカード"
      EndIf
      TextGadget(#GADGET_Text,38*3,top,100,15,t$)   
...
KanjiChart:
    ...
    If CreateGadgetList(WindowID())
      ...
      Else 
        t$ = "英語"
      EndIf
      CheckBoxGadget(#GADGET_CheckBox, 120, Top, 130, 20, t$) :Top + 30
      SetGadgetState(#GADGET_CheckBox,1)
        ...
        Else
          AddGadgetItem(#GADGET_ComboBox,-1,"番号")    
          AddGadgetItem(#GADGET_ComboBox,-1,"ありふれた漢字 1")   
          AddGadgetItem(#GADGET_ComboBox,-1,"ありふれた漢字 2")  
          AddGadgetItem(#GADGET_ComboBox,-1,"家族と人々") 
          AddGadgetItem(#GADGET_ComboBox,-1,"動詞 1") 
          AddGadgetItem(#GADGET_ComboBox,-1,"ありふれた漢字 3") 
          AddGadgetItem(#GADGET_ComboBox,-1,"体") 
        EndIf       
        ...
        Else 
          t$ = "漢字チャート"
        EndIf
        TextGadget(#GADGET_Text,WindowWidth/2-70, top-45,200,15,t$+": "+GetGadgetText(#GADGET_ComboBox))
        If WindowWidth < 500 Or WindowHeight < 500
          ResizeWindow(500,500)
        EndIf 
        k = 0
        For y = top To 38*6+top Step 38
          For x = left To 38*4+left Step 38
            If k > cards 
              Break
            EndIf 
            ButtonImageGadget(#GADGET_Image + k ,x, y ,36,36,UseImage(kanjistart+k))
            k+1 
          Next 
        Next 
    Else
      MessageRequester(title$,"Unable to open the Kanji Chart!",#MB_ICONERROR)
    EndIf 
        ...
        Select EventGadgetID()           
          Case #GADGET_Image+l
            ...
            If LanguageMode = 0
              ...
              EndIf
            Else ; japanese
              TextGadget(#Gadget_Text+2,38*7,top-30,280,15,"<くん> "+Jname$)
              TextGadget(#Gadget_Text+3,38*7,top-15,280,15,"<おん> "+Jname2$)
              If GetGadgetState(#GADGET_CheckBox) = 1
                TextGadget(#Gadget_Text+4,38*7,top,280,15, "<英語> "+ename$)
              EndIf 
              If other$ <> ""
                TextGadget(#Gadget_Text+5,38*7,top+15,280,15, Jother$)
              EndIf 
            EndIf          
        EndSelect
    EndSelect
        ...
        Case #PB_Event_Gadget  
          ...
          Select EventGadgetID()    
            Case #GADGET_ComboBox
              For k = #GADGET_Image To #GADGET_Image + cards
                If IsGadget(k)
                  FreeGadget(k)
                EndIf
              Next
              ...
              For y = top To 38*6+top Step 38
                ...
                Next 
              Next 
              TextGadget(#GADGET_Text,WindowWidth/2-70, top-45,200,15,"Kanji Chart: "+GetGadgetText(#GADGET_ComboBox))
         EndSelect 
       EndSelect 
...
TextConv:
  ...
  If CreateGadgetList(WindowID())
    ...
    Else 
      t$ = "タイポしてください"
    EndIf 
    TextGadget(#GADGET_Text,left, top,100,20,t$) 
    If LanguageMode = 0
      t$ = "\katakana\"
    Else 
      t$ = "\カタカナ\"
    EndIf 
    TextGadget(#GADGET_Text+1,left+120, top,180,20,t$)   
    If LanguageMode = 0
      t$ = "To Kana"
    Else 
      t$ = "かな"
    EndIf  
    ButtonGadget(#GADGET_Button, left+250, top, 70, 20, t$)         
...
; UseIcon = D:\purebasic\Projects\Japanese_Tutor\misc\JapanIcon.ico
; Executable = Japanese Tutor.exe